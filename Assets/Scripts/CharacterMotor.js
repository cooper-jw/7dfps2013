...
class CharacterMotorMovement 
{
	...
	var currentMaxForwardSpeed : float = 10.0;
	var currentMaxSidewaysSpeed : float = 10.0;
	var currentMaxBackwardsSpeed : float = 10.0;
	var maxWalkForwardSpeed : float = 10.0;
	var maxSidewaysSpeed : float = 10.0;
	var maxBackwardsSpeed : float = 10.0;
	var maxForwardSprintSpeed : float = 15.0;
	var sprintMultiplier : float = 1.5;
	
	// Curve for multiplying speed based on slope (negative = downwards)
	var slopeSpeedMultiplier : AnimationCurve = AnimationCurve(Keyframe(-90, 1), Keyframe(0, 1), Keyframe(90, 0));
	
	// How fast does the character change speeds?  Higher is faster.
	var currentMaxGroundAcceleration : float = 30.0;
	var maxWalkAcceleration : float = 30.0;
	var maxSprintAcceleration : float = 40.0;
...
private function UpdateFunction () 
{
	...
	{
		movement.currentMaxForwardSpeed = movement.maxForwardSprintSpeed;
		movement.currentMaxGroundAcceleration = movement.maxSprintAcceleration;
		movement.currentMaxSidewaysSpeed = movement.maxSidewaysSpeed * movement.sprintMultiplier;
		movement.currentMaxBackwardsSpeed = movement.maxBackwardsSpeed * movement.sprintMultiplier;
		
	}else{
		movement.currentMaxForwardSpeed = movement.maxWalkForwardSpeed;
		movement.currentMaxGroundAcceleration = movement.maxWalkAcceleration;
		movement.currentMaxSidewaysSpeed = movement.maxSidewaysSpeed;
		movement.currentMaxBackwardsSpeed = movement.maxBackwardsSpeed;
	}
...
function GetMaxAcceleration (grounded : boolean) : float {
	...
	if (grounded)
		return movement.currentMaxGroundAcceleration;
	else
...
function MaxSpeedInDirection (desiredMovementDirection : Vector3) : float {
	if (desiredMovementDirection == Vector3.zero)
		...
		var zAxisEllipseMultiplier : float = (desiredMovementDirection.z > 0 ? movement.currentMaxForwardSpeed : movement.currentMaxBackwardsSpeed) / movement.currentMaxSidewaysSpeed;
		var temp : Vector3 = new Vector3(desiredMovementDirection.x, 0, desiredMovementDirection.z / zAxisEllipseMultiplier).normalized;
		var length : float = new Vector3(temp.x, 0, temp.z * zAxisEllipseMultiplier).magnitude * movement.currentMaxSidewaysSpeed;
		return length;
	}
}
